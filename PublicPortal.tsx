  ...
  if (selectedId && selected) {
    return (
      <div className="min-h-screen bg-white text-black p-6 md:p-24 animate-in fade-in duration-500">
        <button onClick={() => setSelectedId(null)} className="text-[10px] font-black uppercase mb-12 flex items-center gap-2 tracking-widest hover:text-[#00c073]"><ChevronLeft size={14}/> Return to Terminal</button>
        <div className="max-w-3xl mx-auto">
          ...
          <div className="prose max-w-none text-lg leading-relaxed text-zinc-800 whitespace-pre-line">{selected.content}</div>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12">
        ...
        <section className="lg:col-span-8 p-6 md:p-12 border-r border-zinc-100">
          ...
          <div className="space-y-20">
            {loading ? <div className="animate-pulse text-zinc-300 font-black uppercase tracking-widest">Scanning Vault...</div> : 
             bespoke.map(post => (
              <article key={post.id} onClick={() => setSelectedId(post.id)} className="group cursor-pointer">
                ...
                <div className="flex items-center gap-2 text-[10px] font-black uppercase underline decoration-[#ccff00] underline-offset-8 decoration-2">Open Thesis <ArrowUpRight size={14}/></div>
              </article>
            ))}
          </div>
        </section>
        ...
        <aside className="lg:col-span-4 p-6 md:p-12 bg-zinc-50/50">
          ...
          <div className="space-y-6">
            {wire.map(item => (
              <a key={item.id} href={item.source_link} target="_blank" rel="noopener noreferrer" className="block group border-b border-zinc-200 pb-6 last:border-0 hover:bg-white p-4 -mx-4 transition-all">
                ...
                <h4 className="text-base font-bold leading-snug group-hover:underline decoration-[#ccff00]">{item.title}</h4>
              </a>
            ))}
          </div>
        </aside>
